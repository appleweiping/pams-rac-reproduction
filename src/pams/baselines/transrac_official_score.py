"""Sealed scorer for strict official-asset TransRAC prediction artifacts.

Prediction and receipt bytes are completely validated before the canonical
development target loader is called.  The scorer therefore forms the second
process in the label firewall established by
:mod:`pams.baselines.transrac_official_runner`.
"""
...
EVALUATION_CLASSIFICATION = (
    "TransRAC official checkpoint / modern compatibility / local 84-dev sanity"
)
CANONICAL_DEV_ID_SHA256 = (
    "2199294a1d22da6c67d2fdaaafb4bebaa11e92a5bb3accd4670975815001f2c6"
)
SCORING_CODE_RELATIVE_PATH = "src/pams/baselines/transrac_official_score.py"
...
class TransRACOfficialScoreError(RuntimeError):
...
class ValidatedLabelFreeInputs:
    ...
    def assert_unchanged(self) -> None:
        if _stable_file_digest(self.sidecar_path) != self.sidecar_digest:
            raise TransRACOfficialScoreError(
                "label-free sidecar changed during TransRAC scoring"
            )
        if _stable_file_digest(self.commitment_path) != self.commitment_digest:
            raise TransRACOfficialScoreError(
                "label-free commitment changed during TransRAC scoring"
            )
...
def _load_label_free_scoring_inputs(
    ...
    if (
        ...
        raise ValueError("TransRAC scoring requires the exact 84-video dev sidecar")
    observed_sidecar = _stable_file_digest(sidecar_path)
    observed_commitment = _stable_file_digest(commitment_path)
    if observed_sidecar != sidecar_digest:
        raise TransRACOfficialScoreError(
            "label-free sidecar changed while it was validated"
        )
    if observed_commitment != commitment_digest:
        raise TransRACOfficialScoreError(
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            if key.lower() in FORBIDDEN_KEYS:
                raise ValueError(
                    f"TransRAC prediction contains forbidden label field {key!r}"
                )
...
def _load_json_strict(path: Path, expected_sha256: str) -> Any:
    ...
    if __import__("hashlib").sha256(raw).hexdigest() != expected_sha256:
        raise TransRACOfficialScoreError(f"file changed between hashing and parsing: {path}")
    ...
    *,
    ...
    root = _strict_object(
        prediction,
        {
            ...
            "method_id",
            "classification",
            "eligible_for_original_pams_transrac_cell",
            "labels_loaded",
            "scoring_performed",
            "runner_provenance",
            "source",
    ...
    if (
        ...
        raise ValueError("TransRAC prediction method/schema classification mismatch")
    if (
        root["eligible_for_original_pams_transrac_cell"] is not False
        or root["labels_loaded"] is not False
        or root["scoring_performed"] is not False
    ):
        raise ValueError("TransRAC prediction crossed or misrepresented the label boundary")
    ...
    if runner_provenance != {
        ...
        raise ValueError(
            "TransRAC prediction was not produced by this clean runner revision"
        )
    ...
    if source != {
        ...
        raise ValueError("TransRAC source binding is not frozen")
    ...
    input_binding = _strict_object(
        ...
        "TransRAC input binding",
    )
    if input_binding != label_free_inputs.input_binding:
        raise ValueError(
            "TransRAC prediction input binding does not match the supplied "
            "label-free sidecar/commitment"
        )
    ...
    if (
        ...
        raise ValueError("TransRAC scoring requires 84 unique selected IDs")
    video_ids = tuple(ids)
    manifest_records = label_free_inputs.manifest.records
    if video_ids != tuple(record.video_id for record in manifest_records):
        raise ValueError(
            "TransRAC selected IDs/order differ from the verified label-free sidecar"
        )
    if _canonical_dev_id_sha256(video_ids) != CANONICAL_DEV_ID_SHA256:
        raise ValueError("TransRAC selected IDs are not the frozen dev identities")
    if selection["selected_video_ids_sha256"] != sha256_json(list(video_ids)):
        raise ValueError("TransRAC selection digest mismatch")
    ...
    if (
        ...
        raise ValueError("TransRAC asset identity mismatch")

    config = _strict_object(root["config"], {"sha256", "values"}, "TransRAC config")
    if (
        config["values"] != FROZEN_TRANSRAC_OFFICIAL_CONFIG.to_dict()
        or config["sha256"] != FROZEN_TRANSRAC_OFFICIAL_CONFIG.fingerprint
    ):
        raise ValueError("TransRAC config is not frozen")
    ...
    if (
        ...
        raise ValueError("TransRAC runtime provenance is incomplete")

    restore = _strict_object(
        root["restore_audit"],
        {
            "checkpoint_epoch",
            "checkpoint_filename_epoch_token",
            "state_dict_keys",
            "loaded_key_count",
            "missing_keys",
            "unexpected_keys",
    ...
    if restore != {
        ...
        raise ValueError("TransRAC checkpoint restore audit mismatch")
    if (
        root["decode_ok_total"] != 84
        or root["failure_total"] != 0
        or root["failures"] != []
    ):
        raise ValueError("TransRAC dev prediction contains failures")

    rows = root["predictions"]
    if not isinstance(rows, list) or len(rows) != 84:
        raise ValueError("TransRAC dev prediction requires exactly 84 rows")
    ...
    for index, (raw_row, manifest_record) in enumerate(
        ...
        row = _strict_object(raw_row, row_fields, f"TransRAC row {index}")
        video_id = row["video_id"]
        locator = row["video_locator"]
        if not isinstance(video_id, str) or not video_id:
            raise ValueError(f"TransRAC row {index} has invalid video_id")
        if not isinstance(locator, str) or "\\" in locator:
            raise ValueError(f"TransRAC row {index} has invalid video_locator")
        locator_path = PurePosixPath(locator)
        if locator_path.is_absolute() or any(
            part in {"", ".", ".."} for part in locator_path.parts
        ):
            raise ValueError(f"TransRAC row {index} has unsafe video_locator")
        expected_video = _sha256(
            row["expected_video_sha256"], f"rows[{index}].expected_video_sha256"
        )
        if (
            video_id != manifest_record.video_id
            or locator != manifest_record.video_path
            or expected_video != manifest_record.video_sha256
        ):
            raise ValueError(
                f"TransRAC row {index} does not match the verified sidecar record"
            )
        if (
            _sha256(
            ...
            raise ValueError(f"TransRAC row {index} source-video hash mismatch")
        if row["decode_status"] != "ok" or row["failure_reason"] is not None:
            raise ValueError(f"TransRAC row {index} is not successful")
        ...
        if (
            ...
            raise ValueError(f"TransRAC row {index} rounded count mismatch")
        if (
            ...
            raise ValueError(f"TransRAC row {index} frame metadata mismatch")
    ...
    if tuple(prediction_ids) != video_ids:
        raise ValueError("TransRAC prediction order differs from its selection")
    ...
    expected_receipt = {
        ...
        "config_sha256": FROZEN_TRANSRAC_OFFICIAL_CONFIG.fingerprint,
        "compatibility_image_digest": COMPATIBILITY_IMAGE_DIGEST,
    ...
    if receipt_root != expected_receipt:
        raise ValueError("TransRAC prediction receipt does not bind the prediction")
    if _stable_file_digest(prediction_path) != prediction_digest:
        raise TransRACOfficialScoreError("prediction changed during validation")
    if _stable_file_digest(receipt_path) != receipt_digest:
        raise TransRACOfficialScoreError("prediction receipt changed during validation")
...
def paired_bootstrap(
    ...
    *,
    ...
    values: dict[str, list[float]] = {
        key: [] for key in compute_transrac_metrics(raw, rounded, targets)
    }
    for _ in range(samples):
        indices = [generator.randrange(len(targets)) for _ in targets]
        metrics = compute_transrac_metrics(
    ...
    return {
        key: {"low": _percentile(metric, 0.025), "high": _percentile(metric, 0.975)}
...
def _clean_git_revision(repository_root: Path) -> str:
    try:
        ...
        raise TransRACOfficialScoreError("unable to audit scoring Git checkout") from exc
    if GIT_SHA_PATTERN.fullmatch(revision) is None or status:
        raise TransRACOfficialScoreError(
            "sealed TransRAC scoring requires a clean full-SHA Git checkout"
        )
    return revision


def score_transrac_dev(
    *,
    ...
    """Validate predictions/receipt first, then open and score dev targets."""
    ...
    if evaluation_path.exists() or receipt_path.exists():
        raise FileExistsError("refusing to overwrite TransRAC score artifacts")

    # The target path is not opened before every operation in this block succeeds.
    ...
    if scoring_code != Path(__file__).resolve(strict=True):
        raise TransRACOfficialScoreError("executed scorer is outside repository_root")
    ...
    expected_runner_module = Path(__file__).with_name(
        "transrac_official_runner.py"
    ).resolve(strict=True)
    if runner_code != expected_runner_module:
        raise TransRACOfficialScoreError(
            "bound TransRAC runner is outside repository_root"
        )
    ...
    if target_ids != predictions.video_ids:
        raise ValueError("TransRAC prediction/target IDs or order differ")
    ...
    if _stable_file_digest(target_file) != target_digest:
        raise TransRACOfficialScoreError("dev targets changed during scoring")
    if _stable_file_digest(prediction_file).sha256 != predictions.prediction_sha256:
        raise TransRACOfficialScoreError("prediction artifact changed during scoring")
    if (
    ...
    if _stable_file_digest(scoring_code) != scoring_code_digest:
        raise TransRACOfficialScoreError("TransRAC scoring code changed during scoring")
    if _stable_file_digest(runner_code) != runner_code_digest:
        raise TransRACOfficialScoreError("TransRAC runner code changed during scoring")
    if _clean_git_revision(repository) != scoring_git_sha:
        raise TransRACOfficialScoreError("scoring Git revision changed during scoring")
    label_free_inputs.assert_unchanged()

    evaluation = {
        "schema_version": 1,
        "artifact_type": "transrac_official_dev_evaluation",
        ...
        "prediction_boundary": {"labels_loaded": False, "scoring_performed": False},
        "metrics": metrics,
        "bootstrap": {
            "samples": 10_000,
            "seed": 2026,
    ...
    receipt = {
        "schema_version": 1,
        "artifact_type": "transrac_official_dev_evaluation_receipt",
        "classification": EVALUATION_CLASSIFICATION,
        "eligible_for_original_paper_table": False,
    ...
    try:
        ...
        if not _remove_artifact_if_exact(evaluation_path, created_evaluation):
            raise RuntimeError(
                "TransRAC score receipt write failed and the exact orphan "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m pams.baselines.transrac_official_score",
        description="Score a fully validated TransRAC label-free prediction receipt.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (OSError, TransRACOfficialScoreError, ValueError) as exc:
