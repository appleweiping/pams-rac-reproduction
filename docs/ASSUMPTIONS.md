...
| Person policy | legacy MediaPipe is single-person; choose a dominant candidate if supplied, crop only before the first and after the last detection (`crop_to_detected_span=true`), and retain every internal miss as an exact-zero masked frame | inferred | This is not multi-person identity tracking; decode/detection failures use one published rule |
