...
| Official segment input (v1) | convert annotation `start_frame`/`end_frame` from 1-based inclusive to 0-based half-open and retain the complete interval timeline | recovered from the official UCFRep annotation/loader protocol | Per-`.mat` SHA-256 and clip range enter dataset, label-free sidecar, checkpoint-training, and cache identities; cycle boundaries remain privileged |
| Official segment decode shortfall | default `error`; explicit `pad_invalid_tail` appends invalid-mask suffix frames only | independent auditable closure aligned with the official loader's zero-padding behavior | Policy, expected/decoded/padded frame totals and clip range enter schema-v3 cache metadata and extraction ledger; no silent truncation |
...
| Temporal shape (legacy v2/v3 and official-segment v1 caches) | uniform resampling to 256 frames | inferred historical closure | Original FPS and frame count remain in metadata; existing cache identities are unchanged |
| Temporal shape (v4a pose recovery) | `none_native_timeline`; output length is exactly `clip_end_frame - clip_start_frame`, with explicit invalid tail padding after early EOF and no interpolation across internal misses | recovered UCFRep loader range/padding semantics plus strong evidence from the PAMS supplement's native long-sequence plots (including activity/pause structure around frames 800–1000); not exact author code | Separate pose/config fingerprint; batching may append exact-zero, mask-false frames to the batch maximum but never changes native indices, FPS, or period scale |
...
| Period range | 4–128 frames in the historical 256-frame training/readout configs; preserve fractional FFT-bin period until integer indexing is required | inferred | Avoids quantizing values such as 256/40=6.4 before consensus; these bounds are not yet authorized for native-timeline v4a training |
| V4a authorization boundary | the frozen v4a gate evaluates train337 pose recovery only; its inherited period 4–128, position mode, and training blocks are placeholders outside that authorization | independent safety closure | A passing pose gate cannot be reported as a native-timeline PAMS baseline; dynamic native-period settings require a new fingerprint and gate before training |
...
| Reference-relative SSHead | opt-in projected-pose reference prototype; negative squared-distance teacher; reference + fundamental + lag + .1 low-frequency + .01 smoothness | independently inferred repair after the legacy head-collapse diagnosis | Distinct config fingerprint; target-free; same builder in training/inference; never claim as author-disclosed PAMS |
