...
def test_environment_and_retrospective_result_state_claim_boundaries() -> None:
    ...
    assert result["status"] == (
        "complete_legacy_run_requires_current_runner_rerun"
    )
    compatibility = result["current_code_compatibility"]
    assert compatibility["legacy_artifact_generated_by_current_runner"] is False
    assert compatibility["tested_with_current_loader"] is False
    assert compatibility["compatible"] is False
    assert compatibility["requires_rerun"] is True
