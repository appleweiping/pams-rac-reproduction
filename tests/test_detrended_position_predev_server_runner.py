...
RUNNER = (
    ...
    / "scripts/server/run_pams_position_acf_detrended_predev_seed2026.sh"
)
READOUT = (
    Path(__file__).parents[1]
    / "configs/readouts/projected_position_acf_linear_detrend_predev_v1.yaml"
)

READOUT_SHA256 = "6226f2b4c3a8c61240e7a7b7f9226f21fdbd8cf8464780fada2235bd3700094f"
READOUT_SEMANTIC_SHA256 = (
    "7ebbfa0409e9b6908ccaac78099056b39882e5d4966f330f03fe0c419e3910b0"
)
...
def test_container_gate_uses_only_detrended_api_and_fixed_batches() -> None:
    source = _runner()
    container_python = _heredocs(source, "CONTAINERPY")

    assert len(container_python) == 1
    gate = container_python[0]
    assert "estimate_period_from_detrended_projected_position" in gate
    assert "projected_position_detrended_vector_acf_diagnostics" in gate
    ...
    assert "tuple(range(2, 65))" in gate
    assert '{str(index): 0 for index in range(2, 65)}' in gate


def test_synthetic_then_train337_gates_are_complete_and_target_free() -> None:
    source = _runner()
    ...
    train_index = gate.index("train337 = train337_gate(", synthetic_index)
    assert synthetic_index < train_index
    for condition in (
...
def test_artifact_contains_provenance_hardware_histograms_and_mount_audit() -> None:
    ...
    assert '"selected_bin_histogram": complete_histogram(rows)' in gate
