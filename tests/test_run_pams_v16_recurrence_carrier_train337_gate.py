...
from scripts.server import run_pams_v16_recurrence_carrier_train337_gate as runner
...
def _passing_inputs() -> dict[str, object]:
    return {
        ...
        "readout_distribution": {
            ...
            "selected_count_active_period_reference_absolute_gap": _summary(1.0),
        },
        "count_distribution": {
            "zero_share": 0.24,
            "mode_share": 0.24,
        ...
        "corruption_controls": {
            "time_shuffle": {
                ...
                "shuffled_to_baseline_gate_energy_median_ratio": 0.75,
            },
            "zero_pose": {
                "positive_period_confidence_share": 0.05,
                "curve_available_share": 0.05,
                "active_support_share": 0.05,
            },
        },
    }
...
def test_all_sixteen_criteria_are_required_and_every_eval_stays_closed() -> None:
    passing = _passing_inputs()
    decision = runner._gate_decision(**passing)

    assert len(decision["criteria"]) == 16
    assert decision["all_sixteen_criteria_pass"] is True
    assert decision["overall_pass"] is True
    assert decision["isolated_dev84_prediction_authorized"] is False
    assert decision["paper_baseline_dev84_prediction_authorized"] is False
    assert decision["legacy_resampled_v3_mechanism_only"] is True
    ...
    failures: tuple[tuple[tuple[str, ...], float], ...] = (
        ...
        (
            ("corruption_controls", "zero_pose", "active_support_share"),
            0.051,
        ),
    )
...
def test_exact_candidate_config_is_single_field_checkpoint_compatible() -> None:
    ...
    candidate_path = (
        ...
        / "configs"
        / "experiments"
        / "pams_noabs_projected_teacher_v16_recurrence_carrier_v2.yaml"
    )
...
def test_artifact_receipt_is_exclusive_hash_bound_and_test_closed(
    ...
    digest = "a" * 64
    payload = {
        "status": "legacy_resampled_v3_mechanism_gate_pass",
        "gate": {
            "overall_pass": True,
    ...
    assert receipt["isolated_dev84_prediction_authorized"] is False
    assert receipt["paper_baseline_dev84_prediction_authorized"] is False
    assert receipt["legacy_resampled_v3_mechanism_only"] is True
