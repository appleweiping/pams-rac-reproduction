...
RUNNER = ROOT / "scripts/server/run_pams_pose_recovery_v4b_pilot.sh"
GATE = ROOT / "configs/gates/pams_pose_recovery_v4b_pilot.yaml"
CONFIG = ROOT / "configs/experiments/pams_pose_recovery_v4b.yaml"


def test_v4b_pilot_runner_is_server_only_and_mounts_no_privileged_split() -> None:
...
def test_v4b_pilot_audit_container_has_no_source_videos() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    ...
    assert "--v4b-ledger /pams/v4b-ledger.json" in audit_create
    assert "--output /pams/audit/projected-gate.json" in audit_create


def test_v4b_pilot_gate_binds_config_and_unchanged_thresholds() -> None:
    ...
    assert gate["artifact_type"] == (
        "pams_pose_recovery_v4b_train337_long_tail_pilot_gate"
    )
    assert bindings["v4b_config_file_sha256"] == hashlib.sha256(
        CONFIG.read_bytes()
    ).hexdigest()
    assert bindings["v4b_config_fingerprint"] == config.fingerprint
    assert bindings["v4b_pose_fingerprint"] == config.pose_fingerprint
