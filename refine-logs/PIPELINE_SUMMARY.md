...
**Evidence:** prospective partial-cache pilot; zero eligible results  
...
- Execution status: Gates 0–2 have not run, so training remains blocked.
...
1. `G0-PACK-CPU` — deterministic hash-before-unpickle and physical isolation gate.
2. `G1-FIXTURE-CPU` — only after Gate 0 passes; dual-environment/numerical fixture gate.
3. `G2-SELECTOR-CPU` — only after Gate 1 passes; immutable fixture and all-training-identity selector gate.

No training run is currently authorized.
