...
> **Current state:** zero eligible results; final verdict `REVISE`; same-family provisional; K8 `BLOCKED`; training `BLOCKED`.  
...
|---|---|---|---|---|---:|---|---|
| G0-PACK-CPU | CPU gate | Hash-before-unpickle, feature/vault/audit split, whitelist, eligibility, all-nine components, import separation | Contract frozen | `cpu-local` | 0 GPUh | K6/K7 on any failure | TODO |
...
| G2-SELECTOR-CPU | CPU gate | Exactly 1,000 fixtures plus every eligible training identity and global weak-view receipt | G1 PASS | `cpu-local` | 0 GPUh | Global K5 on any failure | BLOCKED |
| G3-SMOKE-CPU | CPU smoke | CLI/import/prediction schema before tiny GPU | G0–G2 PASS | `cpu-local` | 0 GPUh | Stop before GPU on failure | BLOCKED |
...
1. `G0-PACK-CPU` — TODO.
2. `G1-FIXTURE-CPU` — BLOCKED on Gate 0.
3. `G2-SELECTOR-CPU` — BLOCKED on Gate 1.

No training run is currently launchable. K8 remains externally blocked and no result field is populated.
