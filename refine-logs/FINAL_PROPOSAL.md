# Research Proposal: WARP-PHASE — Discrete Warp-Integral Equivariance for Supplied-Track MRAC
...
**Current status:** Final prospective proposal after the five-round research-refine limit; not implemented; **zero eligible results**; **same-family provisional**. Gates 0–2 have not run, training remains blocked, and no efficacy or novelty claim is licensed. The unresolved execution and novelty preconditions are frozen below.

## 15. Proposal Status and Unresolved Preconditions

- **Evidence ceiling:** Proposal-only; zero eligible method results. Historical v46/v62/v63 artifacts remain ineligible as efficacy evidence.
- **Local execution blocker 1 — weak rejection view:** Freeze one deterministic Gate-2 RNG stream, checksum-sorted track and parent-start order, exact draw shapes/order, and stored perturbation hashes.
- **Local execution blocker 2 — fixture generator/environment:** Complete the 1,000-fixture generator's RNG positions, x/y harmonic equations, symmetric templates, stochastic array shapes, and transform order; then either make the project's locked NumPy 1.26.4 environment normative or bind a dedicated NumPy 2.4.6 environment with a cross-environment byte/hash witness.
- **Local execution blocker 3 — K4 constructors:** Define the exact numerator and denominator of `f_track`, the matched-time shuffle scope/permutation/seed, and the unseen-resampler kernel, target grid, schedule, seed, and receipt without changing the four K4 ratios, six Stage-B jobs, or three validation blocks.
- **External novelty blocker — K8:** The complete TWCRAC primary source has not been obtained; full-text coverage is zero pages, `k8.decision=NOT_PASSED_BLOCKED`, `claim_freeze=BLOCKED`, and `novelty_clearance=false`. Abstract-only evidence cannot establish collision or distinctness.
- **Authorization:** Training cannot begin until the local contracts and Gates 0–2 pass. Claim freeze additionally requires complete-primary-source TWCRAC adjudication and all reached K-rules, audit, and result-to-claim gates.
