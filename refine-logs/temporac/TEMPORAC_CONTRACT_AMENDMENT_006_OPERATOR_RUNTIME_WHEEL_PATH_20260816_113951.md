...
**Scope:** one runtime-path correction plus the minimum additive provenance
binding needed to prevent a later candidate from binding only the known-broken
Amendment 004 runtime. This amendment creates no candidate, implementation,
test, gate receipt, experiment, result, or authority.
...
additively overrides Amendment 004 in exactly two places for every subsequent
operator candidate:

1. the read-only wheel bind target and all bootstrap references to the wheel;
...
Exactly four substitutions are permitted in the Amendment 004 runtime object:
...
SHA-256
`71fdf4bfe78b59aada9f2e207002eae3fcd10345ed1f47ee9969557d6649f5da`.
...
retained inside those 3,976 bytes so the only runtime-byte delta is the wheel
path and derived bootstrap hash. For a post-Amendment-006 candidate receipt v3,
Section 5 below additively overrides only that provenance count from 24 to 28;
the source-snapshot framing algorithm and tag remain unchanged. This narrow
precedence rule resolves the otherwise contradictory historical count without
altering the corrected runtime bytes.
...
digest, Python or NumPy version, imported `__init__.py` or installed `RECORD`
commitment, network/root/tmpfs policy, environment, install flags, generator
argv/output, algorithm, manifest schema, manifest row, row order, aggregate,
fixture population, NOLA, decoder, threshold, job, seed, GPU hour, dataset,
claim, or paper-visible evidence block.
...
current generator, and the two current operator test entrypoints. These are
historical inputs to this proposed correction, not expected hashes of a future
implementation. Any future candidate must bind the actual reviewed
