# TempoRAC Contract Amendment 005 - Certificate Provenance and Closed K7 Replay (Round 6)

Status: PROPOSED_PENDING_ROUND6_REVIEW  
Family: same-family provisional  
Authority: all zero  
Generated: 2026-08-16T18:43:03+08:00

This document is the complete Round-6 replacement for the rejected Round-5 Amendment005 candidate. It is a provenance specification, not an implementation, accepted review, gate receipt, test run, experiment, or authorization. It closes only fresh-review findings A005-R5-B1, A005-R5-B2, A005-R5-B3, and A005-R5-B4 while preserving the already closed 10+64 CPython initialization maps, remaining FD contract, 16-key association contract, 54-owner/106-edge graph, and every scientific/interface rule. It does not alter the dominant method, algorithm, data, claims, thresholds, exact 27 training jobs, 93 planned A6000-hours, 402-identity population, 9,648 prediction envelopes, seven-member target artifact, ten-key target receipt, fourteen-key G5a receipt, capability semantics, or evaluator metrics.
...
Only a fresh independent Round-6 ACCEPT of the exact timestamped MD/JSON pair may permit a later separately authorized implementation and review. If accepted, this amendment refines the canonical proposal only where it speaks expressly. All unmentioned canonical rules remain unchanged.
...
The fresh Round-5 review that requires this revision is bound exactly in lineage:

- MD: 19,671 bytes, ca3807657ee4197996c49c270cabd73bb43e439422173ca780849f433113a6f7;
- JSON: 17,110 bytes, 8f2490fcfe8434e1d3f2adc3a2a5db7afb4166302f421fa06e45512b764cf2ff.

The rejected Round-5 fixed pair is preserved byte-identically under new archive timestamp 20260816_184302:

- MD: 269,255 bytes, dbf797e80156d627b14290dfe558222878fdec8ccefbf28a051093130e1349e4;
- JSON: 226,592 bytes, 94fc8e5db1031686ea0d5e5646c94606728c18cff19915a6deb231a3bf80cfcb.

The original Round-5 timestamped pair at 20260816_173527 has those same hashes and remains unchanged. Earlier rejected archives, accepted-input reviews, and reviewer traces remain bound in the JSON lineage and are not effective-contract constituents.

The complete current local non-self snapshot remains the exact 57-path Round-5 binding set: three runtime-bootstrap files, 24 TempoRAC source files, 19 TempoRAC tests, and eleven planning/review files. The Round-5 review pair above is an additional exact revision input in lineage, not a silent replacement inside that 57-path set. A generation-time full rehash matched all 57 declared digests, including tests/temporac/test_gates_fixtures.py d6d91a7dd606c42f672bb015d77c5c10406c2c6de69ed3a8bff76a0ce4f713d3 and tests/temporac/test_operator_manifest.py 7f2c90c320d7378a74fcc64f6c808eb6e426cdcfbd73cab0fb1242f690db9db0.
...
2. this exact timestamped Round-6 MD at suffix 20260816_184303;
3. this exact timestamped Round-6 JSON at suffix 20260816_184303;
4. future exact accepted Round-6 review MD;
5. future exact accepted Round-6 review JSON.
...
accepted effective contract -> pre-environment supervisor/CPython/initial-ELF/init/FD ledgers -> exact discovery capsule -> durable zero-process precommit -> two sequential fresh bootstrap discoveries -> exact two-row discovery-evidence index/root plus byte-identical nonempty output/root -> trusted-generated index -> final source/wheel/installed/environment indexes -> distinct full supervisor execution -> typed post-run trace evidence.
...
## 6. A005-R4-B1/B2/B3 plus A005-R5-B1/B2/B3/B4 - executable native runtime and unique provenance bytes
...
A sealed memfd follows one executable construction order. The controller calls memfd_create with exact MFD_ALLOW_SEALING|MFD_CLOEXEC, promotes the writable descriptor to an FD_CLOEXEC descriptor at least 64, closes/proves-EBADF the raw descriptor, ftruncates, writes once with pwrite, and verifies exact size, offset zero, zero initial seals, payload bytes/hash and EOF. While that writer remains open it adds exactly F_SEAL_GROW|F_SEAL_SEAL|F_SEAL_SHRINK|F_SEAL_WRITE and reads the exact seal mask back. No writable mapping or second writer exists.

Still while the writer is open, the controller opens exact /proc/self/fd/<base10-writer-fd> once with O_RDONLY|O_CLOEXEC, promotes the reader to a distinct descriptor at least 64, and verifies O_RDONLY|O_LARGEFILE, common regular-file device/inode/size/seals, exact payload/hash/EOF, and independent open-file-description offsets. It then closes the writer, requires F_GETFD=-1/EBADF on it, repeats every surviving-reader check, dup3(reader,child_fd,0), closes/proves-EBADF the reader, and verifies that the prescribed child FD alone is read-only, offset zero, byte/seal/inode exact and inheritable for exec. Reopening after writer close, duplicating the writer, sharing offsets, disk fallback, mmap, writable child input, or reordered/omitted work fails.

temporac.sealed-memfd-construction-readback.v4 records exactly eight ordered stages: writer-created, writer-populated-verified, writer-sealed, reader-opened, ofd-independence-proved, writer-closed, reader-postclose-verified, and child-fd-installed. One temporac.sealed-memfd-construction-index.v4 binds the exact standalone readback bytes/hash for discovery fd0 or full-runtime fds 0,3,4,5 in numeric child-FD order. The exact launch instance embeds that index's complete bytes as lowercase hex plus SHA-256, so transient proof is committed before exec without changing the preserved 20-key FD row or 15-key child readback. Fourteen exact error-stage tokens cover create/promote/truncate/write/verify/seal/open/offset/close/dup/final verification; any syscall error, EINTR, short operation, wrong errno or readback closes all row descriptors, creates no launch/evidence object, and terminates the controller failure path.

Each /dev/null is a separately opened character device major 1/minor 3 with O_WRONLY|O_LARGEFILE, offset/st_size zero. Pipes are O_WRONLY, nonseekable ESPIPE, exact capacity, zero queued bytes at entry, one controller read peer, and terminal EOF only after the joined final writer closes.
...
Before any discovery exec, P06 durably commits temporac.generated-source-discovery-precommit.v4 with exact capsule bytes/hash, contract, schema, and created_before_process_count=0. The controller then launches two sequential fresh exec processes, run_id/process_ordinal 0 then 1, with distinct pid/starttime pairs and distinct pipes. Row 0 is drained to EOF, joined with exit zero, attested and committed before any run-1 pipe or process is created. Each child imports exactly dataclasses as the sole requested module and allows only CPython-stdlib transitive imports. The hook captures the complete finite nonempty ordered sequence of every non-file-backed source compile immediately paired with exactly one matching execution; exact CPython 3.12.13 dataclasses -> inspect -> collections.namedtuple guarantees at least one row. No caller dataclass application, pams/third-party import, dispatch, final-index access, predicted row, empty list, or unmatched compile/exec is allowed.

Each child writes one independent FD3 stream: ASCII temporac.generated-source-discovery.fd3.v4, NUL, uint16_be(5), then exactly five frames. A frame is uint8 type, uint64_be payload length, raw32 payload SHA-256, and complete canonical JSON-plus-LF payload. Types 1..5 carry exec-entry FD readback, locked-pre-cpython FD readback, post-pyinitialize FD readback, complete initialization readback, and discovery output. Every payload is positive and at most 16 MiB; the stream is at most 64 MiB. Truncation, trailing bytes, alternate endian, wrong digest/order/schema, concatenated JSON, shared stream or absent terminal EOF fails.

temporac.generated-source-discovery-output.v4 has exactly capsule_sha256, rows, schema. Each row has the eleven exact generator/source fields in the mirror. Canonical output bytes and SHA-256 are recomputed; output_root=H(temporac.generated-source-discovery-root.v4,raw32(capsule_sha256),exact output bytes).

temporac.generated-source-discovery-evidence-index.v4 is one standalone canonical index with exactly two rows ordered run_id 0 then 1. Each 23-key row binds exact launch-instance bytes/hash, configuration-readback bytes/hash, all three FD-readback bytes/hashes, discovery-output bytes/hash/root, complete FD3 stream byte count/hash, capsule/precommit/supervisor hashes, process identity and a standalone joined attestation bytes/hash. The attestation has exactly 22 keys, names each of the three FD-stage digests independently, and requires exited/0, signal null, wait status zero, FD3 EOF true, JOINED_PASS and exact cross-digest equality. The two complete outputs/hashes/roots and normalized semantic launch/config/readbacks must agree under the sole enumerated instance-specific differences; pid/starttime, run ordinal and fresh pipe/process evidence must differ. The index SHA-256 hashes its complete canonical bytes including LF, and discovery_evidence_root=H(temporac.generated-source-discovery-evidence-root.v4,exact complete index bytes including LF). Neither shared output nor a digest list substitutes for that preimage. Both digest and tagged root bind the trusted index and final environment. Equality is evidence, not authorization of either producer.

Only after the two-row evidence index/root validates is the trusted-generated index derived. Its row array is exactly the common nonempty eleven-key discovery row array, byte-for-byte and in ordinal order. Full runtime imports dataclasses first. For every ordinal it must emit one dedicated PY_TRUSTED_GENERATED_COMPILE immediately followed by one dedicated PY_TRUSTED_GENERATED_EXEC with the byte-identical row-derived origin, consuming that row exactly once; generic PY_CODE_EVAL/PY_CODE_EXEC is never an alternative for trusted-generated origin. Only after ordinals 0..R-1 are consumed may it import pams and the ASCII runtime closure. Missing, extra, reordered, repeated, unmatched, project/third-party or post-DISPATCH_BEGIN generated action fails. After the trusted index and final environment exist, a distinct --embedded-cpython-v4 P06 invocation runs ordinary dispatch/receipt flow and binds the final trace. There is no discovery/final-environment cycle.
...
Each kind's exact key list is in the mirror. Installed origins repeat distribution/version/member/wheel/installed/repository identity and resolve one association. Native/process origins repeat absolute path, tagged build-id null/present state, ELF hash, launcher/launch instance, provider class, and tagged installed-association null/present state.

Generated origins repeat the complete discovery/trusted row identity. trusted_row_sha256 is SHA-256 of one exact standalone object with exactly the trusted row's eleven keys, including generated_source_hex, serialized as canonical compact recursively sorted-key UTF-8 JSON plus exactly one LF. It excludes parent/index bytes, output-root tags, ordinal prefixes, H framing, schema and its own digest. That standalone object must parse/re-encode byte-identically and be the unique row in both discovery outputs and the trusted index; collision or alternate encoding fails.

The sole trusted class/kind mapping is generator_origin_class cpython-stdlib-source to origin.generator_origin_kind cpython-stdlib. trusted-row generator_sha256 equals origin.generator_origin_sha256 byte-for-byte and equals exactly one CPython runtime-member stdlib-source digest; renaming, rehashing or substitution is forbidden. The origin's profile, ordinal, source length/hash, compile filename/mode, generator qualname/digest, discovery output root and trusted row digest reconstruct exactly one row. Readback origins repeat launch instance, profile, schema, stage, positive bytes, and complete hash.
...
Round 6 rehashes all 57 current non-self binding paths described in Section 2. No test, review, implementation, launch, or experiment was executed during this authoring revision.
...
A005-R5-B1 attacks close-before-reopen, reopen after EBADF, reader duplication from the writer, shared offsets, wrong inode/seals/flags/EOF, surviving writer, writable or CLOEXEC child FD, missing/reordered construction stages/readback, and every alternate error token. Positives execute all eight construction stages with the writer open through the /proc reopen, prove distinct open-file descriptions and common inode, close/prove-EBADF the writer, reverify the reader, install the exact read-only child FD, and commit its standalone readback through the launch instance.

A005-R5-B2 attacks every two-row invariant: missing/extra/reordered run, reused process or pipe, overlapping processes, shared/multiplexed FD3, malformed header/type/length/endian/digest, missing launch/config/FD stage/output/attestation bytes, truncation/trailing bytes, absent EOF/join, nonzero exit, swapped readback, or evidence digest without exact index/root. Positives use two sequential fresh exec processes and two independent five-frame streams; both joined attestations, all exact byte objects and the complete canonical evidence index/root validate before trusted/environment construction.

A005-R5-B3 attacks an empty trusted set, a subset/superset/different row, out-of-order or non-once consumption, generic PY_CODE_EVAL/PY_CODE_EXEC substitution, missing adjacency, unmatched compile/exec, project/third-party generated action, and post-DISPATCH generation. Positives require the common nonempty discovery rows and exactly one adjacent dedicated trusted compile-to-exec pair for every ordinal 0..R-1.

A005-R5-B4 attacks omission of LF or generated_source_hex from trusted_row_sha256, parent/root/tag/ordinal preimages, alternate JSON, zero/multiple row resolution, class/kind mismatch, generator digest rename/rehash/substitution, or any failed origin projection. Positives round-trip each exact standalone eleven-key row-plus-LF and the sole cpython-stdlib-source -> cpython-stdlib, generator_sha256 -> generator_origin_sha256 mapping.
...
| finding | Round-6 closure |
|---|---|
| A005-R5-B1 | executable writer-open /proc reopen; independent O_RDONLY OFD proof; writer close/EBADF; reader postclose verification; exact child dup; eight-stage construction readback and fourteen-stage error contract bound by launch instance |
| A005-R5-B2 | two sequential fresh discovery processes; independent five-frame FD3 streams; exact launch/config/three-FD-stage/output/joined-attestation bytes in a two-row canonical evidence index; direct SHA and tagged root bound by trusted index/environment |
| A005-R5-B3 | trusted set exactly equals both complete nonempty discovery row arrays; every full runtime consumes ordinals once through adjacent dedicated compile-to-exec pairs, with generic and post-dispatch alternatives forbidden |
| A005-R5-B4 | SHA-256 of exact standalone eleven-key trusted row plus LF; unique row resolution; sole generator class/kind and digest equality; complete origin projection and parse/re-encode equality |
...
Every row is PROPOSED_PENDING_ROUND6_REVIEW, not accepted authority.

## 16. Final disposition

This amendment remains same-family provisional with every authority value zero. It preserves the rejected Round-5 fixed pair byte-identically at timestamp 20260816_184302, creates an exact Round-6 timestamped pair at 20260816_184303, and uses fixed names only as byte-identical aliases. A fresh independent Round-6 review is required before effective-contract construction or implementation.
...
The following fenced payload is byte-for-byte the standalone Round-6 JSON file, including its single terminal LF before the closing fence.
...
```json
{
  ...
  "disposition": {
    "final_status": "PROPOSED_PENDING_ROUND6_REVIEW",
    "implementation_required_after_acceptance": "Only after an independent Round6 ACCEPT may a separately authorized implementation/review create the effective-contract index, complete CPython initialization profile, FD construction ledgers, discovery precommit/capsule/two-run evidence/output, trusted rows, typed installed-member associations, populated CPU environment, native supervisor and launch instances, executable-origin traces, artifacts, receipts, indexes, tests, and replay evidence. Current source/tests remain a bound non-authorizing snapshot.",
    "no_actions_taken": [
    ...
    "review_required": "A fresh independent Round6 specification review must inspect the exact timestamped MD/JSON pair and verify A005-R5-B1, A005-R5-B2, A005-R5-B3, and A005-R5-B4 while confirming the 10+64 initialization maps, remaining FD contract, 16-key association, 54-owner/106-edge DAG and every already closed scientific/interface semantic remain unchanged before any implementation, acceptance-derived authority, or execution."
  },
  "effective_contract": {
    ...
    "construction": [
      "The index does not exist until an independent fresh Round6 review accepts this exact timestamped amendment pair.",
    ...
    "role_order": [
      ...
      {
        "path": "refine-logs/temporac/TEMPORAC_CONTRACT_AMENDMENT_005_CERTIFICATE_PROVENANCE_20260816_184303.md",
        "role": "amendment_markdown"
      },
      {
        "path": "refine-logs/temporac/TEMPORAC_CONTRACT_AMENDMENT_005_CERTIFICATE_PROVENANCE_20260816_184303.json",
        "role": "amendment_json"
      },
      {
  ...
  "generated_at": "2026-08-16T18:43:03+08:00",
  "k1_closed_outcome": {
    "coverage": [
