...
`w=H^T rho/||H^T rho||`. Define the exact dimensionless scale
`s_X0=25/6`. The clean orbit is
...
in-place accumulation. Only after the complete bracket exists, execute one
outer multiplication by `np.float64(25.0/6.0)`. The analytic tangent follows
the identical rule: finish the fundamental and increasing-harmonic
accumulation, then multiply the complete tangent once by the same scalar.
Distributing or reassociating the scalar, scaling basis vectors, or changing
the MGS inputs is forbidden.
...
For each separation `d`, RMS is the binary64 RMS over the complete exact
`4096 x 34` array of COCO-17 `x,y` coordinate differences; the 18 fixed
coordinates remain in the denominator as exact zero differences. Production
evaluates `pose=G(phi)` and `shifted=G((phi+1/d) mod 1)` as C-contiguous
binary64 arrays, forms `difference=pose-shifted`, calls
`np.square(difference)`, calls `np.mean(...,dtype=np.float64)` over all
`4096*34` C-order elements, casts the result once to Python `float`, and then
calls `math.sqrt`. The reversal predicate retains the literal C-order flatten,
reversed-grid indexing, `np.fft.rfft`, conjugate product, `np.fft.irfft`,
energy, nonnegative clamp, and minimum sequence; a closed-form reversal value
is an audit witness only, not a replacement implementation. Attempt 0..63 is
tried in order; the first passing attempt is retained.
...
Define categorical indices in the frozen orders by
`aidx(0.60/0.75/1.00)=0/1/2` and
`lidx(interior/left-boundary/right-boundary/run-reset)=0/1/2/3`. For row
`(g,w,a,o,l)`, the exact 10-byte key is

`K=uint16_be(g)||uint16_be(w)||uint16_be(aidx(a))||uint16_be(o)
   ||uint16_be(lidx(l))`.

For zero-based edge index `e`, define

`P(e)=ASCII("temporac.operator.v4")||0x00||uint64_be(10)||K||uint32_be(e)`,
`D(e)=SHA256(P(e))`, and `b(e)=unsigned(D(e)[0])`.

There is exactly one `uint64_be(10)` length prefix around `K`, no per-field
length, no length around `uint32_be(e)`, no `H()` wrapper, no digest cycling,
and no `e mod 32` selection. All integers are range-checked before encoding.
Let `RN-even-f32(x)` be direct ties-to-even rounding of exact real `x` to
binary32. The inactive response is

`inactive(e)=min(RN-even-f32((255+3*b(e))/2550),
                  float32_bits(0x3ecccccc))`.

Production forms the exact integer numerator, divides once in binary64, casts
once to NumPy little-endian `<f4`, applies the exact binary32 cap, and retains
that scalar. The 1,024-byte little-endian lookup in increasing `b=0...255`
order has SHA-256
`afa6eff0d744acfdf9cf78adc43aa8daf01633f3345d7567d642a7782885040d`.
Only `b=255` differs from the uncapped correctly rounded table: committed bits
are `0x3ecccccc` and little-endian bytes `cccccc3e`.

For plateau `[s,s+w)`, the lower-middle edge is
`s+floor((w-1)/2)`. Exact ordinary/boosted binary32 bit pairs for amplitudes
`0.60`, `0.75`, and `1.00` are respectively
`(0x3f19999a,0x3f266666)`, `(0x3f400000,0x3f4ccccd)`, and
`(0x3f800000,0x3f800000)`. Every other plateau edge uses the ordinary value.
Thus amplitude-1.00 width-2 and width-4 plateaus are flat at exact binary32
one; no unique-maximum property is required or claimed for them.

Responses are one-dimensional little-endian `<f4` arrays in increasing edge
order. The frozen raw witness roots are:

- `SHA256(concat_r(uint64_be(10)||K_r)) =
  18dfdafe237b77565de36bab6d4642efb314ec1cbbd76e852938ed5c4d424a55`
  over 186,624 bytes;
- `SHA256(concat_r(D_r(0))) =
  ba797f6ae59cbbb40f19a61199afaaa1ac5b2c001a3c9fc34f1dba8f79cf78e4`
  over 331,776 raw digest bytes; and
- the 746,714-byte canonical semantic-row JSON SHA-256 is
