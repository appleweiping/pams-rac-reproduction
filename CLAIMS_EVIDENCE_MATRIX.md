...
|---|---|---|---|---|---:|---|
| C01 | The paper's exact scope is identity-indexed, window-local tempo routing for multi-person repetition counting. | Use the working title *TempoRAC: Identity-Indexed Local Tempo Routing for Multi-Person Repetition Counting*; do not imply primacy. | User-approved ICASSP plan; `NARRATIVE_REPORT.md` | PROVISIONAL | false | Reconcile the title against synchronized implementation and final experiment scope. |
