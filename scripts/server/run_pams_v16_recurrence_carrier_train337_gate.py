"""Gate the v16 recurrence-gated quadrature carrier on train337 only.
...
This legacy-resampled-v3 mechanism gate cannot authorize a paper-baseline
dev84 prediction, scoring, or sealed test105 access.  A later author-timebase
candidate must bind a native dense cache and retrained encoder independently.
"""
...
_THRESHOLDS: Mapping[str, float] = MappingProxyType(
    {
        ...
        "selected_count_active_reference_gap_median_maximum": 1.0,
...
class _CandidateSample:
    ...
    count: int
    reference_count: int
    full_timeline_reference_count: int
...
def _finite_float(value: Tensor | float) -> float:
    ...
    if not math.isfinite(result):
        raise RuntimeError("recurrence-carrier gate produced a non-finite value")
    return result
...
def _validated_counter(config: PAMSConfig) -> MultiExpertCounter:
    ...
    if consensus.expert_mode != "multi":
        raise ValueError("recurrence-carrier candidate requires all three experts")
    ...
    with torch.inference_mode():
        for start in range(0, len(sequences), batch_size):
            ...
            for index, video_id in enumerate(batch.video_ids):
                ...
                mask = readout.active_masks[index, :length]
                result = counter.count(
                    ...
                    reference_count_override=int(
                        readout.active_reference_counts[index]
                    ),
                )
                ...
                samples.append(
                    _CandidateSample(
                        ...
                        active_support_fraction=_finite_float(
                            readout.active_support_fractions[index]
                        ),
                        recurrence_gate_energy=_finite_float(
                            readout.recurrence_gate_energies[index]
                        ),
                        phase_offset=_finite_float(readout.phase_offsets[index]),
...
def _readout_distribution(samples: Sequence[_CandidateSample]) -> dict[str, Any]:
    ...
    support_fractions = np.asarray(
        [sample.active_support_fraction for sample in samples], dtype=np.float64
    )
    gate_energies = np.asarray(
        [sample.recurrence_gate_energy for sample in samples], dtype=np.float64
    )
    active_reference_gaps = np.asarray(
        [abs(sample.count - sample.reference_count) for sample in samples],
        dtype=np.float64,
    )
    full_reference_gaps = np.asarray(
        [
            abs(sample.count - sample.full_timeline_reference_count)
    ...
    return {
        ...
        "active_support_fraction": _summary(support_fractions),
        "recurrence_gate_energy": _summary(gate_energies),
        ...
        "selected_count_active_period_reference_absolute_gap": _summary(
            active_reference_gaps
        ),
        "selected_count_full_timeline_reference_absolute_gap_non_authorizing": (
            _summary(full_reference_gaps)
        ),
            ...
            period_rows.append(
                {
                    ...
                    "baseline_active_support_fraction": (
                        source.active_support_fraction
                    ),
                    "scaled_active_support_fraction": (
                        candidate.active_support_fraction
                    ),
                    "eligible": eligible,
            ...
            count_rows.append(
                {
                    ...
                    "baseline_active_reference_count": source.reference_count,
                    "scaled_active_reference_count": candidate.reference_count,
...
def _corruption_controls(
    ...
    *,
    ...
    baseline_gate_energy: list[float] = []
    shuffled_gate_energy: list[float] = []
    baseline_active_support: list[float] = []
    shuffled_active_support: list[float] = []
    zero_confidences: list[float] = []
    zero_available: list[bool] = []
    zero_curve_stds: list[float] = []
    zero_active_support: list[float] = []
    model.eval()
    with torch.inference_mode():
        for start in range(0, len(sequences), batch_size):
            batch = collate_pose_sequences(sequences[start : start + batch_size]).to(device)
            embeddings = model.encoder(batch.poses, batch.valid_mask)
            baseline = estimate_recurrence_carrier_curves(
                embeddings,
                minimum_period=config.period.minimum,
                maximum_period=config.period.maximum,
                valid_mask=batch.valid_mask,
            ...
            shuffled = build_recurrence_carrier_curves(
                shuffled_embeddings,
                baseline.periods,
                batch.valid_mask,
                timeline_lengths=batch.lengths,
                period_confidences=baseline.period_confidences,
            )
            ...
            baseline_gate_energy.extend(
                _finite_float(value) for value in baseline.recurrence_gate_energies
            )
            shuffled_gate_energy.extend(
                _finite_float(value) for value in shuffled.recurrence_gate_energies
            )
            baseline_active_support.extend(
                _finite_float(value) for value in baseline.active_support_fractions
            )
            shuffled_active_support.extend(
                _finite_float(value) for value in shuffled.active_support_fractions
            )
            ...
            zero_active_support.extend(
                _finite_float(value)
                for value in zero_readout.active_support_fractions
            )
    ...
    baseline_gate_summary = _summary(baseline_gate_energy)
    shuffled_gate_summary = _summary(shuffled_gate_energy)
    baseline_gate_median = _median(baseline_gate_summary)
    shuffled_gate_median = _median(shuffled_gate_summary)
    gate_energy_ratio = (
        None
        if (
            baseline_gate_median is None
            or baseline_gate_median <= 1e-12
            or shuffled_gate_median is None
        )
        else shuffled_gate_median / baseline_gate_median
    )
    return {
        "time_shuffle": {
            "algorithm": (
            ...
            "baseline_recurrence_gate_energy": baseline_gate_summary,
            "shuffled_recurrence_gate_energy": shuffled_gate_summary,
            "shuffled_to_baseline_gate_energy_median_ratio": gate_energy_ratio,
            "baseline_active_support_fraction": _summary(
                baseline_active_support
            ),
            "shuffled_active_support_fraction": _summary(
                shuffled_active_support
            ),
        },
        "zero_pose": {
            "algorithm": (
                "replace every pose coordinate by zero while retaining the exact "
                "dense valid mask, then rerun encoder, ACF, recurrence, and carrier"
            ),
            ...
            "active_support_fraction": _summary(zero_active_support),
            "active_support_share": float(
                np.mean(np.asarray(zero_active_support) > 0.0)
            ),
        },
    }
...
def _gate_decision(
    *,
    ...
    criteria = {
        ...
        "selected_count_active_period_reference_gap_median": _v14._criterion(
            _median(
                readout_distribution[
                    "selected_count_active_period_reference_absolute_gap"
                ]
            ),
            operator="<=",
            threshold=_THRESHOLDS[
                "selected_count_active_reference_gap_median_maximum"
            ],
        ),
        ...
        "time_shuffle_gate_energy_median_ratio": _v14._criterion(
            corruption_controls["time_shuffle"][
                "shuffled_to_baseline_gate_energy_median_ratio"
            ],
            operator="<=",
            threshold=_THRESHOLDS[
                "time_shuffle_gate_energy_median_ratio_maximum"
            ],
        ),
        ...
        "zero_pose_active_support_share": _v14._criterion(
            float(corruption_controls["zero_pose"]["active_support_share"]),
            operator="<=",
            threshold=_THRESHOLDS[
                "zero_pose_active_support_share_maximum"
            ],
        ),
    }
    ...
    return {
        ...
        "criteria": criteria,
        "all_sixteen_criteria_pass": passed,
        "overall_pass": passed,
        "isolated_dev84_prediction_authorized": False,
        "paper_baseline_dev84_prediction_authorized": False,
        "legacy_resampled_v3_mechanism_only": True,
    ...
    *,
    ...
    paths = {
        ...
        "recurrence_carrier_module": Path(
            estimate_recurrence_carrier_curves.__code__.co_filename
        ),
    ...
    if gate_code_source_git_sha != algorithm_source_git_sha:
        raise RuntimeError("gate and recurrence-carrier source revisions differ")
    ...
    if stage != "encoder":
        raise ValueError("recurrence-carrier gate requires an encoder checkpoint")
    if len(provenance.training_video_ids) != _EXPECTED_TRAINING_VIDEOS:
        raise ValueError("recurrence-carrier gate requires exactly train337")
    validate_terminal_checkpoint(
    ...
    if clean_git_revision(Path.cwd()) != algorithm_source_git_sha:
        raise RuntimeError("recurrence-carrier source changed during gate")
    ...
    payload = {
        ...
        "status": (
            "legacy_resampled_v3_mechanism_gate_pass"
        ...
        "label_firewall": {
            "accepted_scientific_inputs": [
                ...
                "single_field_recurrence_carrier_candidate_config",
        ...
        "algorithm": {
            ...
            "recurrence_positive_lag": "one frozen ACF period",
            "recurrence_negative_lags": ["half period", "one-and-a-half periods"],
            "invalid_frames": (
                "excluded from statistics and exact zero; arbitrary native length "
                "supported; holes and pauses are not compacted or bridged"
            ),
            "counter": (
                "unchanged three peak experts and majority-first fallback; all experts "
                "and the analytic reference share the same recurrence-active spans"
            ),
            "final_count_is_expert_count": True,
            "reference_count_timebase": (
                "embedding-conditioned carrier maxima inside recurrence-active spans"
            ),
            "full_timeline_reference": "reported only as a non-authorizing diagnostic",
        },
        ...
        "samples": [
            {
                ...
                "active_support_fraction": sample.active_support_fraction,
                "recurrence_gate_energy": sample.recurrence_gate_energy,
                "phase_offset": sample.phase_offset,
                "curve_available": sample.curve_available,
                "count": sample.count,
                "reference_count": sample.reference_count,
                "full_timeline_reference_count_non_authorizing": (
                    sample.full_timeline_reference_count
                ),
        ...
        "scientific_caveats": [
            ...
            (
                "A formal baseline candidate requires an official-segment native dense "
                "v4 pose cache and an encoder retrained on that exact timebase."
            ),
            "Passing self-consistency does not establish repetition-count accuracy.",
            "No result from this gate can authorize dev84 or test105 access.",
        ],
        "hardware": hardware,
