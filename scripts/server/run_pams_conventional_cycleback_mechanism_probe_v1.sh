#!/usr/bin/env bash
# Thin identity-free mechanism entrypoint; predecessors are canonical outcome records.
set -Eeuo pipefail
IFS=$'\n\t'
umask 027

fail() { printf 'cycleback-mechanism-probe-v1: %s\n' "$*" >&2; exit 2; }
...
for forbidden in \
  ...
  PAMS_CYCLEBACK_GEOMETRY_RUN_ROOT PAMS_CYCLEBACK_GEOMETRY_GATE_SHA256 \
  PAMS_CYCLEBACK_GEOMETRY_RECEIPT_SHA256 PAMS_CYCLEBACK_LAUNCH_REGISTRY_ROOT \
  PAMS_ROOT; do
...
exec /usr/bin/env -i \
  ...
  --stage mechanism --attempt-id "$ATTEMPT_ID" --candidate-id "$CANDIDATE_ID"
