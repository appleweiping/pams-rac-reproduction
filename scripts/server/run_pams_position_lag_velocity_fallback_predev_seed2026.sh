...
# detrended projected-position lag-ACF with velocity fallback. Exactly one network-isolated GPU container
...
readonly READOUT_CONFIG_SHA256="b3c38341e442765cb709d8ca9c5ecfe541121ea61d1ae35c0ff2cb48b6b09a98"
readonly READOUT_CONFIG_SEMANTIC_SHA256="e17b4105f732be1bc4a40b153c0db28729fc0379a5f921ab0a26d3b2301b4233"
...
readonly RUN_PARENT="${ROOT}/runs/pams-position-lag-velocity-fallback-predev-seed2026"
...
readonly GATE_SCRIPT="${AUDIT_ROOT}/lag-velocity-fallback-predev.py"
...
readonly CONTAINER_NAME="plagvelfallback-${ATTEMPT_ID}"
...
[[ "$RUN_PARENT_RESOLVED" == "${ROOT}/runs/pams-position-lag-velocity-fallback-predev-seed2026" ]] \
...
def load_policy(path):
    ...
    expected_readout = {
        "input_source": "encoder_input_projection_pre_positional_encoding",
        "primary_temporal_transform": (
            "mask_aware_per_feature_least_squares_linear_detrend"
        ),
        "primary_evidence": (
            "masked_centered_vector_dot_product_fft_autocorrelation"
        ),
        "primary_lag_minimum_frames": 4,
        "primary_lag_maximum_frames": "min_128_and_floor_valid_length_over_2",
        "primary_peak_finder": "scipy_find_peaks",
        "primary_peak_minimum_distance_frames": 4,
        "primary_peak_minimum_prominence": 0,
        "primary_peak_height_guard": "strictly_positive",
        "primary_near_best_height_fraction": 0.90,
        "primary_selection": (
            "smallest_lag_at_least_90_percent_of_maximum_positive_peak_height"
        ),
        "fallback_trigger": "no_positive_primary_local_peak",
        "fallback": "projected_pose_pairwise_valid_velocity_vector_acf_spectrum",
        "fallback_selection": "maximum_allowed_non_dc_power_share",
        "fallback_confidence_guard": "strictly_positive",
        "no_evidence_period_frames": 4,
        "minimum_period_frames": 4,
        "maximum_period_frames": 128,
        "count_duration": "valid_frame_intervals",
...
def stable_period_key(period):
    return format(float(period), ".9g")


def complete_period_histogram(rows):
    histogram = {str(index): 0 for index in range(4, 129)}
    histogram["none"] = 0
    for row in rows:
        key = (
            stable_period_key(row["period_frames"])
...
def complete_selection_source_counts(rows):
    counts = {
        "detrended-position-lag-acf": 0,
        "projected-velocity-spectrum-fallback": 0,
        "no-evidence": 0,
    }
    for row in rows:
        source = row["selection_source"]
        if source not in counts:
            raise RuntimeError("unknown selection source: " + str(source))
        counts[source] += 1
    return counts
    ...
    with torch.inference_mode():
        for start in range(0, len(items), 32):
            ...
            if len(diagnostics) != len(batch.video_ids):
                raise RuntimeError("lag-velocity diagnostic batch mismatch")
            ...
            for index, video_id in enumerate(batch.video_ids):
                ...
                if int(diagnostic.valid_length) != valid_frames:
                    raise RuntimeError("lag-velocity diagnostic valid length mismatch")
                if not math.isclose(
                    period,
                    ...
                    raise RuntimeError("lag-velocity estimator/diagnostic period mismatch")
                if not math.isclose(
                    confidence,
                    ...
                    raise RuntimeError(
                        "lag-velocity estimator/diagnostic confidence mismatch"
                    )
                if diagnostic.selection_source not in {
                    "detrended-position-lag-acf",
                    "projected-velocity-spectrum-fallback",
                    "no-evidence",
                }:
                    raise RuntimeError("lag-velocity selection source drifted")
                if diagnostic.selected_lag is not None and (
                ...
                rows.append(
                    {
                        ...
                        "selected_lag": diagnostic.selected_lag,
                        "selection_source": diagnostic.selection_source,
                        "fallback_period_frames": float(
                            diagnostic.fallback_period
                        ),
                        "fallback_confidence": float(
                            diagnostic.fallback_confidence
                        ),
                    }
                )
...
def synthetic_gate(policy, model, config, device):
    ...
    for harmonic in harmonic_orders:
        ...
        sequences.append(
            PoseSequence(
                video_id=f"harmonic-order-{harmonic}",
                fps=30.0,
                xyz=xyz,
    ...
    for row, harmonic in zip(rows, harmonic_orders, strict=True):
    ...
    return {
        ...
        "summary": {
            ...
            "selected_period_histogram": complete_period_histogram(rows),
            "selection_source_counts": complete_selection_source_counts(rows),
        },
        "rows": rows,
    }


def synthetic_count_sweep_gate(policy, model, config, device):
    gate = policy["predev_gates"]["synthetic_count_sweep"]
    counts = tuple(int(value) for value in gate["counts"])
    if counts != tuple(range(2, 41)):
        raise ValueError("synthetic count sweep must be exactly count=2..40")
    frames = int(gate["frames"])
    drift = float(gate["linear_drift_per_frame"])
    if (
        frames != 256
        or gate["signal"] != "two_dimensional_sine_cosine_quadrature"
        or drift != 1.0
    ):
        raise ValueError("synthetic count-sweep gate constants drifted")
    time = np.arange(frames, dtype=np.float64)
    duration = float(frames - 1)
    sequences = []
    expected_periods = []
    for count in counts:
        phase = 2.0 * np.pi * float(count) * time / duration
        xyz = np.zeros((frames, 33, 3), dtype=np.float32)
        xyz[:, 0, 0] = (np.sin(phase) + drift * time).astype(np.float32)
        xyz[:, 0, 1] = (
            np.cos(phase) - 0.5 * drift * time
        ).astype(np.float32)
        sequences.append(
            PoseSequence(
                video_id=f"count-sweep-{count}",
                fps=30.0,
                xyz=xyz,
                valid_mask=np.ones(frames, dtype=np.bool_),
            )
        )
        expected_periods.append(duration / float(count))
    rows, batch_sizes = infer(tuple(sequences), model, config, device)
    relative_errors = []
    for row, count, expected_period in zip(
        rows,
        counts,
        expected_periods,
        strict=True,
        ...
        relative_error = (
            abs(float(row["period_frames"]) - expected_period)
            / expected_period
        )
        row["expected_count"] = count
        row["expected_period_frames"] = expected_period
        row["relative_period_error"] = relative_error
        row["linear_drift_per_frame"] = drift
    ...
    positive = all(float(row["confidence"]) > 0.0 for row in rows)
    maximum_error = float(gate["maximum_relative_period_error"])
    recovered_fraction = sum(
        error <= maximum_error for error in relative_errors
    ) / max(1, len(relative_errors))
    conditions = {
        "record_total_exact": len(rows) == int(gate["required_records"]),
        "fixed_batch_32_with_tail": batch_sizes == [32, 7],
        "all_outputs_finite": finite if gate["require_finite_outputs"] else True,
        "all_confidences_positive": (
            positive if gate["require_positive_confidence"] else True
        ),
        "maximum_relative_period_error_lte_frozen_maximum": (
            max(relative_errors, default=math.inf) <= maximum_error
        ),
        "recovered_fraction_gte_frozen_minimum": (
            recovered_fraction >= float(gate["minimum_recovered_fraction"])
        ),
    }
    return {
        "status": "passed" if all(conditions.values()) else "failed",
        ...
        "summary": {
            ...
            "frames": frames,
            "linear_drift_per_frame": drift,
            "maximum_relative_period_error": max(
                relative_errors,
                default=math.inf,
            ),
...
def train337_gate(policy, model, config, device, manifest_path, pose_cache_dir):
    ...
    period_counts = Counter(
        stable_period_key(row["period_frames"]) for row in positive
    )
    mode_fraction = max(period_counts.values(), default=0) / denominator
    zero_guard = all(
        row["rounded_count"] == 0
        for row in rows
    ...
    conditions = {
        ...
        "selected_period_mode_fraction_lte_frozen_maximum": (
            mode_fraction
            <= float(gate["maximum_selected_period_mode_fraction"])
        ),
        "unique_selected_periods_gte_frozen_minimum": (
            len(period_counts) >= int(gate["minimum_unique_selected_periods"])
        ),
        "zero_confidence_zero_count": (
            zero_guard if gate["require_zero_confidence_zero_count"] else True
        ),
    }
    histogram = complete_period_histogram(rows)
    return {
        "status": "passed" if all(conditions.values()) else "failed",
        "conditions": conditions,
        "summary": {
            ...
            "selected_period_mode_fraction": mode_fraction,
            "unique_selected_periods": len(period_counts),
            "selected_period_histogram": histogram,
            "selected_period_histogram_key_format": (
                "finite_float_9_significant_digits_or_none"
            ),
            "selection_source_counts": complete_selection_source_counts(rows),
...
def main():
    ...
    synthetic_passed = (
        ...
        and synthetic_count_sweep["status"] == "passed"
    )
    if synthetic_passed:
        ...
        train337 = {
            ...
            "summary": {
                "record_total": 0,
                "selected_period_histogram": complete_period_histogram(()),
                "selection_source_counts": (
                    complete_selection_source_counts(())
                ),
            },
            "rows": [],
        }
    passed = synthetic_passed and train337["status"] == "passed"
    payload = {
        "schema_version": 1,
        "artifact_type": "pams_position_lag_velocity_fallback_predev",
        ...
        "algorithm": (
            ...
            "detrend -> masked vector lag ACF -> earliest peak within 90% "
            "of maximum positive peak; projected-velocity spectrum fallback "
            "only when no positive lag peak exists"
        ),
...
cat > "${RUN_ROOT}/attempt.reservation.json" <<EOF
{
  ...
  "scope": "synthetic-k2-to-k7-and-count2-to40-plus-train337-target-free-only",
  "seed": 2026,
  "source_revision": "${SOURCE_REVISION}",
...
if [[ "$still_running" == "true" ]]; then
  CONTAINER_EXIT=125
elif [[ "$CONTAINER_EXIT" -eq 3 ]]; then
  # Exit 3 is the embedded algorithm's retained predev-gate failure. Docker
  # state is authoritative here so a wrapper attach status cannot relabel the
  # scientific negative as an infrastructure failure.
  :
elif [[ "$attach_exit" -ne "$CONTAINER_EXIT" ]]; then
  CONTAINER_EXIT=125
fi
flock -u 9
...
receipt = {
    "schema_version": 1,
    "artifact_type": "pams_position_lag_velocity_fallback_predev_receipt",
