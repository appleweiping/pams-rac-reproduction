...
Multi-Person Repetition Counting*. Title, abstract, contributions, captions,
and conclusion do not claim the first multi-person, person-wise, asynchronous,
variable-speed, pose-based, self-supervised, or tempo-adaptive counter.
...
no person-wise count or period annotations. The paper separately discloses
supervision and pretraining used by pose estimation, detection, tracking,
selection, calibration, and evaluation.

**Check:** exact-context scan and supervision manifest  
**Evidence:** `method_manifest.yaml`, collaborator training/config artifacts

### AC05 — Abstract result binding

The 100–150-word abstract contains one controlled result sentence. Draft mode
...
`evidence_values.tex`. The abstract contains no citation or undefined acronym.
...
cell names its source or remains visibly pending.
...
bound to one commit/config and paired seed set; absent modules are deleted, not
simulated.
...
curve is presented as measurement.
...
`RACAuthorMetadataComplete=true`, and exact agreement of all names,
affiliations, emails, and order with the submission system.
...
statements, and documented author review against the official 2027 LLM policy.
None is guessed.
...
contributions, captions, results, and conclusion, except clearly attributed
prior-work quotations or explicit negation.
...
current ICASSP source/PDF. The pinned ARIS verifier is executed under
`assurance=submission`; Claude failure caps the outcome at provisional.
