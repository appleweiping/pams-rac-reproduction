...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.97\textwidth]{figures/icassp_framework.pdf}
  \caption{Proposed signal path, not an implementation or result claim.
  Supplied pose tracks and masks retain identity-indexed state while sharing
  encoder and expert parameters. Each window yields local periodic evidence,
  soft routing mixes continuous expert responses, normalized overlap-add
  reconstructs one track response, and one decoder returns each supplied track's
  count. Red synchronization markers identify interfaces that must be replaced
  by source-bound definitions.}
  \label{fig:framework}
\end{figure*}
...
\begin{equation}
\begin{aligned}
 \bX_i &\in \Rset^{T_i\times J\times C},\quad
 m_i\in\{0,1\}^{T_i},\quad
 \widehat{\bc}=[\hat c_1,\ldots,\hat c_N]^{\mathsf T} \\
 \bz_{i\ell}(t) &=
 \cE_\theta\!\left(\bX_i[\cW_{i\ell}],m_i[\cW_{i\ell}]\right)_t,
 \quad t\in\cW_{i\ell}
\end{aligned}
\label{eq:representation}
\end{equation}
\noindent The shared $\theta$ does not mix identity state. Window geometry, pose
...
\begin{aligned}
 ...
 (\hat p_{i\ell},\gamma_{i\ell}) &=
 \Phi\!\left(\{A_{i\ell}(\tau)\}_{\tau\in\cT},
...
\begin{aligned}
 \bg_{i\ell} &=
 \softmax\!\left(
 \rho_\psi(\bz_{i\ell},\hat p_{i\ell},\gamma_{i\ell})/\tau_{\mathrm r}
 \right),\quad
 g_{i\ell k}\geq0,\quad \sum_{k\in\cK}g_{i\ell k}=1 \\
 \br^{(k)}_{i\ell} &=
 \cH^{(k)}_{\phi_k}(\bz_{i\ell}),\quad k\in\cK
\end{aligned}
\label{eq:routing}
\end{equation}
...
\begin{aligned}
 ...
 R_i(t) &=
 \frac{\sum_{\ell:t\in\cW_{i\ell}}
 a_{i\ell}(t)q_{i\ell}(t)\widetilde{r}_{i\ell}(t)}
 {\sum_{\ell:t\in\cW_{i\ell}}
 a_{i\ell}(t)q_{i\ell}(t)+\epsilon} \\
 \widehat{\cE}_i &= \cD(R_i,m_i),\quad
