...
\title{Identity-Indexed Local Tempo Routing for Multi-Person Repetition Counting}

% ICASSP uses single-anonymous review. Draft mode shows an explicit roster
% placeholder. Submission mode receives ignored private metadata through
% submission_wrapper.tex, after the external validator confirms completeness.
\ifsubmission
  \ifdefined\RACAuthorNames
    \name{\RACAuthorNames}
    \address{\RACAuthorAffiliations\\\RACAuthorEmails}
  \else
    \name{AUTHOR ROSTER PENDING}
    \address{AFFILIATIONS PENDING}
  \fi
\else
  \name{AUTHOR ROSTER PENDING}
  \address{Affiliations pending collaborator confirmation}
\fi
