...
**Working title:** *TempoRAC: Identity-Indexed Local Tempo Routing for Multi-Person Repetition Counting*
**Submission format:** four technical pages plus an optional fifth page containing references only; funding/COI and ethical-compliance text stays on pages 1--4
**Review mode:** single-anonymous; Weiping Yan's name, University of Minnesota Twin Cities affiliation, and email are explicitly approved for the tracked draft, while the complete collaborator roster and order remain submission blockers
...
- a prospectively specified experiment and artifact contract;
...
The principal counting metrics are Period-mAP, Period-AP50, Period-AP75,
AvgMAE, and AvgOBO.
Predicted-track protocols additionally retain HOTA, IDF1, and IDSW as frontend
diagnostics. Reproduced learned methods require at least three predeclared
...
1. one main table with separate native/contextual, supplied oracle-track, and
   common predicted-track blocks; the latter two compare Track-PAMS, a
   track-global-tempo control, and the proposed method without cross-block ranking;
2. one ablation table for local/global tempo, uniform/hard/soft routing, no
   expert routing, boxcar fusion, decode-each-window-then-aggregate, NOLA then
   one decode, and only auxiliary losses that exist in the frozen code;
3. one future piecewise time-warp diagnostic generated only after eligible
   results exist, showing local period, routing probability or entropy,
   reconstructed response, and clean-to-corrupt change;
4. two complete PPT-sourced conceptual figures: task motivation and framework.
   Their source/output hashes and captions bind their visual-reference-only
   status, and their embedded media permissions remain `SYNC-REQUIRED`.
...
approximately 550-word experiment and analysis section; and a 100–120-word
conclusion. The body contains the complete introduction and framework figures
from the committed PowerPoint sources plus two compact tables. The data-bound
tempo diagnostic is added only after results exist. Page five contains only references;
funding/COI and the required ethics statement remain on pages 1--4. No appendix,
