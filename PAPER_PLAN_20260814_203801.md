...
reference_page: 5
reference_page_allowed_content: references, funding acknowledgments, compliance with ethical standards
...
page technical limit. The fifth page is restricted to references, funding
acknowledgments, and the Compliance with Ethical Standards statement.
...
| References/funding/ethics | as required | page 5 only | No technical prose, equation, figure, table, appendix, result interpretation, or new method content |
...
- Report Period-mAP, AP50, AP75, AvgMAE, and AvgOBO; retain HOTA, IDF1, and IDSW
...
- First block: native/contextual MultiCounter and MultiCounter+ with their own
  protocol labels; these numbers are not mixed into a fair common-track ranking.
- Second block: common-track Track-PAMS, track-global tempo, and the proposed
  method; include other counters only after output and protocol parity are proven.
...
no expert routing, boundary-reconstruction alternatives, and removal of every
auxiliary loss that actually exists in the synchronized code. Bind every row to
one exact commit/config and use paired same-seed comparisons. Delete conceptual
rows for absent modules rather than simulating them.
...
### Page 5 — References, Funding, and Ethics Only

- Use IEEE numeric citations and only cited, primary-source-verified entries.
- Add funding acknowledgment and Compliance with Ethical Standards text after
  authors supply the exact declarations.
