...
**论文题目：** *TempoRAC: Identity-Indexed Local Tempo Routing for Multi-Person Repetition Counting*
...
约 500 词引言（合并 related work）、约 900 词方法、约 550 词实验、100–120 词
结论，配两张完整 PPT 来源概念图和两张紧凑表；节奏诊断只在真实结果同步后生成。第五页禁止任何方法、实验、
限制、公式、图表、图注或附录内容。

ICASSP 为单盲审稿，最终 PDF 必须包含完整作者顺序、单位和必要联系方式。用户已
明确授权 tracked 草稿显示 Weiping Yan、University of Minnesota Twin Cities 和
`yan00944@umn.edu`；未确认的合作者姓名、顺序、单位仍只允许保存在私有文件中，尚未
...
本轮已完成 ICASSP 2027 英文预结果稿、模块化 LaTeX、16 条已核验且正文实际引用的参考文献、
两张完整 PPT 来源图及其无裁切矢量 PDF、两张结果表壳、方法与结果证据
...
`2976344081abbceb84c4114068bd7b37100ae7aa8438d3112b5227bee442731e`；技术内容在
...
kill-argument 为 `FAIL`：两轮 fresh same-family 审稿分别确认这版作为诚实预结果包通过，
独立攻击与答辩均认为当前包适合作为诚实、可审计的预结果
写作基线，但若现在投稿，会因方法未同步、关键接口不可执行和没有合格 MultiRep
证据而被强拒。最终公式检查确认四组展示公式不含语法标点；结果 schema 也已补上
`table_eligible` 的双向 fail-closed 约束。Claude overlay 最新实时健康探针仍未返回有效 JSON，因此没有跨模型接受
...
也未推送远端。tracked 论文包和 PDF 仅公开了用户明确授权的 Weiping Yan 姓名、UMN
单位与邮箱；未确认的合作者信息仍被阻断。当前仍然缺少四类解锁输入：合作者冻结实现与完整
